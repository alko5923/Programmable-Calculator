...
public class CalculatorParser {
    ...
    private static String NUKE = "nuke";
    ...
    private final ArrayList < String > unallowedVars = new ArrayList < String > (Arrays.asList("Quit",
        "Vars", "Clear", "if"));
    private HashMap<String, FunctionDeclaration> functions = new HashMap<String, FunctionDeclaration>();
    ...
    public SymbolicExpression parse(String inputString, Environment vars, HashMap<String, FunctionDeclaration> _functions) throws IOException {
        this.st = new StreamTokenizer(new StringReader(inputString)); // reads from inputString via stringreader.
        this.vars = vars;
    ...
    /**
     * Checks wether the token read is a conditional expression
     * @return the conditional SymbolicExpression to be evaluated
     * @throws IOException by nextToken() if it reads invalid input
     */
    private SymbolicExpression conditional() throws IOException {
        SymbolicExpression conditional;
        SymbolicExpression left = getCondArg();
        String operation = operation();
        SymbolicExpression right = getCondArg();
        SymbolicExpression ifTrue = getThenElse();
        this.st.nextToken();
        ...
        if (this.st.sval.equals("else")) {
            ...
            throw new SyntaxErrorException("Need an else here!");
        }
        
        
        if (operation.equals("<")) {
            conditional = new LessThan(left, right, ifTrue, ifFalse);
        }
        else if (operation.equals("<=")) {
            conditional = new LessThanOrEqualTo(left, right, ifTrue, ifFalse);
        }
        else if (operation.equals(">")) {
            conditional = new GreaterThan(left, right, ifTrue, ifFalse);
        }
        else if (operation.equals(">=")) {
            conditional = new GreaterThanOrEqualTo(left, right, ifTrue, ifFalse);
        }
        else {
            conditional = new EqualTo(left, right, ifTrue, ifFalse);
        }
    ...
    /**
     * Checks wether the token read is a correct conditional argument
     * @return a SymbolicExpression to be used as conditional argument
     * @throws IOException by nextToken() if it reads invalid input
     */
    private SymbolicExpression getCondArg() throws IOException {
        
        SymbolicExpression arg;
        this.st.nextToken();
        
        if(this.st.ttype == this.st.TT_WORD) {
            if(!vars.containsKey(new Variable(this.st.sval))) {
                throw new SyntaxErrorException("Cannot compare non-assigned variables!");
            }
            ...
            throw new SyntaxErrorException("Need a variable or a number here!");
        }
    }
    
    /**
     * Checks wether the token read is a correction conditional operator
     * @return a String form of the correct conditional operator
     * @throws IOException by nextToken() if it reads invalid input
     */
    private String operation() throws IOException {
        ...
        if (this.st.ttype == '<') {
            ...
            if (this.st.ttype == '=') {
                ...
                throw new SyntaxErrorException("Wrong conditional operator!");
    ...
    /**
     * Checks wether the token read is a valid then/else statement in conditionals
     * @return a SymbolicExpression to be used as then/else statement in conditionals
     * @throws IOException by nextToken() if it reads invalid input
     */
    private SymbolicExpression getThenElse() throws IOException {
        SymbolicExpression thenElse;
        this.st.nextToken();
        if (this.st.ttype == '{') {
            this.st.nextToken();
            thenElse = new Scope(assignment());
            this.st.nextToken();
                if (this.st.ttype == '}') {
                    return thenElse;
                }
                else {
                    throw new SyntaxErrorException("Need a '}' here!");
                }
            }
        else {
            throw new SyntaxErrorException("Need an ifTrue scope here!");
        }
    }
    
    /**
     * Checks wether the token read is correct function declaration
     * @return a Function Declaration SymbolicExpression to be evaluated
     * @throws IOException by nextToken() if it reads invalid input
     */
    private SymbolicExpression functionDeclaration() throws IOException {
        ...
        if (this.st.ttype == this.st.TT_WORD) {
            ...
            throw new SyntaxErrorException("Need a function name here!");
        }
        ...
        if (this.st.ttype == '(') {
            ...
            boolean checkingParams = true;
            
            while (checkingParams) {
                
                this.st.nextToken();
                
                if (this.st.ttype == this.st.TT_WORD) {
                    params.add(new Variable(this.st.sval));
                }
                
                else if (this.st.ttype == this.st.TT_NUMBER) {
                    throw new IllegalExpressionException("Parameters need to be abstract!");
                }
                    ...
                    throw new SyntaxErrorException("Need a comma separating the parameters!");
                }
            }
        }
            
        else {
        throw new SyntaxErrorException("Need some parameters in brackets here!");
                
        }
            
        FunctionBody funcBody = new FunctionBody();
        FunctionDeclaration result = new FunctionDeclaration(funcName, funcBody, params);
        return result;
        
    }
    
    /**
     * Checks wether the token read is a correct function call
     * @return a Function Call SymbolicExpression to be evaluated
     * @throws IOException by nextToken() if it reads invalid input
     */
    private SymbolicExpression functionCall() throws IOException {
        ...
        if (this.st.ttype == '(') {
            ...
            throw new SyntaxErrorException("Need some arguments in brackets here!");
        }
        LinkedList<SymbolicExpression> args = new LinkedList<SymbolicExpression>();
        while (this.st.ttype != ')') {
            this.st.nextToken();
            if (this.st.ttype == this.st.TT_NUMBER) {
                args.add(new Constant(this.st.nval));
                
            }
            if (this.st.ttype == this.st.TT_WORD) {
                args.add(new Variable(this.st.sval));
            }
        }
        FunctionCall result = new FunctionCall(funcName, args);
        return result;
    }
